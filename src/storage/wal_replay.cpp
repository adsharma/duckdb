...
public:
	...
	std::unique_ptr<ParseInfo> DeserializeEntry() {
		if (!deserialize_only) {
			return nullptr;
		}
		deserializer.Begin();
...
protected:
	std::unique_ptr<ParseInfo> ReplayEntry(WALType wal_type);
	std::unique_ptr<ParseInfo> ReplayVersion();
	std::unique_ptr<CreateInfo> ReplayCreateTable();
	std::unique_ptr<DropInfo> ReplayDropTable();
	std::unique_ptr<ParseInfo> ReplayAlter();
	std::unique_ptr<CreateInfo> ReplayCreateView();
	std::unique_ptr<DropInfo> ReplayDropView();
	std::unique_ptr<CreateInfo> ReplayCreateSchema();
	std::unique_ptr<DropInfo> ReplayDropSchema();
	std::unique_ptr<CreateInfo> ReplayCreateType();
	std::unique_ptr<DropInfo> ReplayDropType();
	std::unique_ptr<CreateInfo> ReplayCreateSequence();
	std::unique_ptr<DropInfo> ReplayDropSequence();
	std::unique_ptr<SequenceValueInfo> ReplaySequenceValue();
	std::unique_ptr<CreateInfo> ReplayCreateMacro();
	std::unique_ptr<DropInfo> ReplayDropMacro();
	std::unique_ptr<CreateInfo> ReplayCreateTableMacro();
	std::unique_ptr<DropInfo> ReplayDropTableMacro();
	std::unique_ptr<CreateInfo> ReplayCreateIndex();
	std::unique_ptr<DropInfo> ReplayDropIndex();
	std::unique_ptr<TableDataInfo> ReplayUseTable();
	std::unique_ptr<InsertInfo> ReplayInsert();
	std::unique_ptr<RowGroupDataInfo> ReplayRowGroupData();
	std::unique_ptr<DeleteInfo> ReplayDelete();
	std::unique_ptr<UpdateInfo> ReplayUpdate();
	std::unique_ptr<ParseInfo> ReplayCheckpoint();
...
// Replay Entries
//===--------------------------------------------------------------------===//
std::unique_ptr<ParseInfo> WriteAheadLogDeserializer::ReplayEntry(WALType entry_type) {
	switch (entry_type) {
...
// Replay Version
//===--------------------------------------------------------------------===//
std::unique_ptr<ParseInfo> WriteAheadLogDeserializer::ReplayVersion() {
...
// Replay Table
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateTable() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropTable() {
...
std::unique_ptr<ParseInfo> WriteAheadLogDeserializer::ReplayAlter() {
...
// Replay View
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateView() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropView() {
	auto info = make_uniq<DropInfo>();
...
// Replay Schema
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateSchema() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropSchema() {
	auto info = make_uniq<DropInfo>();
...
// Replay Custom Type
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateType() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropType() {
	auto info = make_uniq<DropInfo>();
...
// Replay Sequence
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateSequence() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropSequence() {
	auto info = make_uniq<DropInfo>();
...
std::unique_ptr<SequenceValueInfo> WriteAheadLogDeserializer::ReplaySequenceValue() {
...
// Replay Macro
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateMacro() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropMacro() {
	auto info = make_uniq<DropInfo>();
...
// Replay Table Macro
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateTableMacro() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropTableMacro() {
	auto info = make_uniq<DropInfo>();
...
// Replay Index
//===--------------------------------------------------------------------===//
std::unique_ptr<CreateInfo> WriteAheadLogDeserializer::ReplayCreateIndex() {
...
std::unique_ptr<DropInfo> WriteAheadLogDeserializer::ReplayDropIndex() {
	auto info = make_uniq<DropInfo>();
...
// Replay Data
//===--------------------------------------------------------------------===//
std::unique_ptr<TableDataInfo> WriteAheadLogDeserializer::ReplayUseTable() {
...
std::unique_ptr<InsertInfo> WriteAheadLogDeserializer::ReplayInsert() {
	auto info = make_uniq<InsertInfo>();
...
std::unique_ptr<RowGroupDataInfo> WriteAheadLogDeserializer::ReplayRowGroupData() {
	...
	if (DeserializeOnly()) {
		...
		auto info = make_uniq<RowGroupDataInfo>();
...
std::unique_ptr<DeleteInfo> WriteAheadLogDeserializer::ReplayDelete() {
	auto info = make_uniq<DeleteInfo>();
	info->chunk = make_uniq<DataChunk>();
	deserializer.ReadObject(101, "chunk", [&](Deserializer &object) { info->chunk->Deserialize(object); });
	if (DeserializeOnly()) {
		return info;
...
std::unique_ptr<UpdateInfo> WriteAheadLogDeserializer::ReplayUpdate() {
	auto info = make_uniq<UpdateInfo>();
...
std::unique_ptr<ParseInfo> WriteAheadLogDeserializer::ReplayCheckpoint() {
...
std::unique_ptr<ParseInfo> WALReader::Next() {
	if (reader->Finished()) {
		return nullptr;
	}
